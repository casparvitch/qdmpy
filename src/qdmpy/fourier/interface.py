...
def get_j_from_bxyz_w_src(
    ...
    bz = copy(bfield[2]) * 1e-4  # copy and convert Gauss to Tesla
    bx_pad, padder = qdmpy.fourier._shared.pad_image(bx, pad_mode, pad_factor)
    by_pad, padder = qdmpy.fourier._shared.pad_image(by, pad_mode, pad_factor)
    bz_pad, padder = qdmpy.fourier._shared.pad_image(bz, pad_mode, pad_factor)
    ...
    fft_s = alpha * kx * fft_bx - alpha * ky * fft_by

    fft_s = Qitool.get_im_filtered(fft_s, "gaussian", sigma=sigma)

    # forward eq: (I = complex unit)
    #   -I ky  I kx
    # {{-----, ----}, {kx, ky}} @ {jx, jy} = {bz, FT{source term}}
    #    a k   a k
    ...
    fft_jx = +1j * alpha * ky * fft_bz + (kx / k) * fft_s
    fft_jy = -1j * alpha * kx * fft_bz + (ky / k) * fft_s
    ...
    corrections = qdmpy.fourier._shared.set_naninf_to_zero(
    ...
    fft_jx = corrections * fft_jx
    fft_jy = corrections * fft_jy
