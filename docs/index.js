...
"url":23,
"doc":"This module contains . Don't import any other qdmpy modules, ensure this is a leaf in the dep. tree. TODO clean this shizzle up. Document, comment, etc. -> add clean test cases/examples"
},
{
...
"url":26,
"doc":"This module holds the Polygon class: a class to compute if a point lies inside/outside/on-side of a polygon. Also defined is a function (polygon_gui) that can be called to select a polygon region on an image. Polygon-GUI      - Function to select polygons on an image. Ensure you have the required gui backends for matplotlib. Best ran seperately/not within jupyter. E.g. open python REpl (python at cmd), 'import qdmpy.itool', then run qdmpy.shared.polygon.Polygonpolygon_gui() & follow the prompts. An optional array (i.e. the image used to define regions) can be passed to polygon_gui. The output json path can then be specified in the usual way (there's an option called 'polygon_nodes_path') to utilize these regions in the main processing code. Polygon    - This is a Python 3 implementation of the Sloan's improved version of the Nordbeck and Rystedt algorithm, published in the paper: SLOAN, S.W. (1985): A point-in-polygon program. Adv. Eng. Software, Vol 7, No. 1, pp 45-47. This class has 1 method (is_inside) that returns the minimum distance to the nearest point of the polygon: If is_inside  0 then point is inside the polygon. Sam Scholten copied from: http: code.activestate.com/recipes/578381-a-point-in-polygon-program-sw-sloan-algorithm/ -> swapped x & y args order (etc.) for image use. Classes    - -  qdmpy.shared.polygon.Polygon Functions     - -  qdmpy.shared.polygon.polygon_selector -  qdmpy.shared.polygon.polygon_gui -  qdmpy.shared.polygon.Polygon "
},
{
