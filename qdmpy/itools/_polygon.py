...
def polygon_gui(image=None):
    ...
    if values["do_load_polys"]:
        from qdmpy.io.json2dict import json_to_dict  # avoid circ. imports...
...
class Widget(object):
    ...
    active = property(
        get_active, lambda self, active: self.set_active(active), doc="Is the widget active?"
    )
...
class PolygonSelector(_SelectorWidget):
    ...
    def __init__(
        ...
        self._polygon_handles = ToolHandles(
            self.ax, self._xs, self._ys, useblit=self.useblit, marker_props=self.markerprops
        )
    ...
    def verts(self):
        ...
        -------
        list
            A list of the vertices of the polygon as ``(xdata, ydata)`` tuples. for each polygon
            (A, B, ...) selected  ``[ [(Ax1, Ay1), (Ax2, Ay2)], [(Bx1, By1), (Bx2, By2)] ]``
        """
